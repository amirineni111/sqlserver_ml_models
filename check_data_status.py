...
def check_data_status():
    ...
    try:
        ...
        if days_old == 0:
            ...
            print(f"  ⚠️  Data is somewhat old ({days_old} days old)")
            needs_retrain = True
        else:
        ...
        print(f"\n💡 RECOMMENDATIONS")
        if needs_retrain:
            print("  🔄 Retraining recommended due to data age")
        ...
        for _, row in sample_data.iterrows():
            ...
            print(f"  {signal_emoji} {row['trading_date'].strftime('%Y-%m-%d')} {row['ticker']} ${close_val:.2f} RSI:{rsi_val:.1f}")
